...
describe("tokenCookie", () => {
  describe("getAccessToken", () => {
    ...
    it("cookies が undefined なら undefined を返す", () => {
      const req = { cookies: undefined } as Parameters<typeof tokenCookie.getAccessToken>[0];
      expect(tokenCookie.getAccessToken(req)).toBeUndefined();
    });
  });
  ...
  describe("setAccessToken", () => {
    it("accessToken を cookie にセットする", () => {
      ...
      expect(res.cookie).toHaveBeenCalledWith("accessToken", "at-789", cookieOptions);
    });
  });
  ...
  describe("setRefreshToken", () => {
    it("refreshToken を cookie にセットする", () => {
      ...
      expect(res.cookie).toHaveBeenCalledWith("refreshToken", "rt-012", cookieOptions);
    });
  });
