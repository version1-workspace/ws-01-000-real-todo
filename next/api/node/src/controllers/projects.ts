...
export const projectsController = {
    ...
    res.json({
      data: result.data.map((entry) =>
        serializeProject(entry.project, {
          milestones: entry.milestones.map((milestone) => serializeTask(milestone)),
          stats: entry.stats,
        }),
      ),
  ...
  async show(req: AuthenticatedRequest, res: Response) {
    ...
    res.json({
      data: serializeProject(result.project, {
        milestones: result.milestones.map((milestone) => serializeTask(milestone)),
        stats: result.stats,
      }),
    });
  },
  ...
  async archive(req: AuthenticatedRequest, res: Response) {
    const { slug } = slugSchema.parse(req.params);
    const project = await projectsService.updateStatus(req.currentUser!.id, slug, 'archived');
  ...
  async reopen(req: AuthenticatedRequest, res: Response) {
    const { slug } = slugSchema.parse(req.params);
    const project = await projectsService.updateStatus(req.currentUser!.id, slug, 'active');
    const stats = await tasksService.stats([project.id]);
    res.json({
      data: serializeProject(project, {
        stats: stats[project.id],
