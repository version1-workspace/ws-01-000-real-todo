...
正規版の実装は `frontend` と `api/node` にあります。`frontend` は Next.js / React / Biome / Vitest / Orval の SPA、`api/node` は Express / Prisma / PostgreSQL の Node.js API です。将来的に Go など別実装を追加する場合は `api/go` のように `api/` 配下へ実装単位で配置します。
