...
- `be.yml`
  `api` を対象に依存関係の導入、Prisma Client 生成、Vitest、build を実行します。
- `e2e.yml`
  PostgreSQL をサービスとして起動し、`api` を build / migrate / seed / 起動したうえで、`api-spec` 配下の TypeScript 製 E2E から `swagger.yaml` の仕様検証を行います。
...
- `.github/actions/prepare-api`
  `api` 用のセットアップと Prisma Client 生成
