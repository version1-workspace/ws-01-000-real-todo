...
- `next-api.yml`
  `next-api` を対象に依存関係の導入、Prisma Client 生成、Vitest、build を実行します。
- `e2e.yml`
  PostgreSQL をサービスとして起動し、`next-api` を build / migrate / seed / 起動したうえで、`api-spec` 配下の TypeScript 製 E2E から `swagger.yaml` の仕様検証を行います。
...
- `.github/actions/prepare-next-api`
  `next-api` 用のセットアップと Prisma Client 生成
