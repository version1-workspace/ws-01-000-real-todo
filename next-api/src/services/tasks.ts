...
export const tasksService = {
  ...
  async update(
    ...
    const data: Prisma.TaskUpdateInput = {
      ...
      ...(values.deadline !== undefined ? { deadline: new Date(values.deadline) } : {}),
      ...(values.finishedAt !== undefined ? { finishedAt: toNullableDate(values.finishedAt) } : {}),
      ...(values.startingAt !== undefined ? { startingAt: toNullableDate(values.startingAt) } : {}),
      ...(values.startedAt !== undefined ? { startedAt: toNullableDate(values.startedAt) } : {}),
      ...(values.archivedAt !== undefined ? { archivedAt: toNullableDate(values.archivedAt) } : {}),
      project: {
  ...
  async milestones(userId: number, projectSlug: string, statuses = ['scheduled']) {
  ...
  async stats(projectIds: number[]) {
    ...
    ) as Record<number, { total: number; kinds: Record<string, number>; states: Record<string, number> }>;

    kindStats.forEach((entry) => {
      const count = typeof entry._count === 'object' && entry._count ? entry._count._all ?? 0 : 0;
